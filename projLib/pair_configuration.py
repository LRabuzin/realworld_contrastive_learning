...
#stratum signifies which dataset an image pair will be assigned to
# 0 - training dataset for the encoder model
# 1 - training dataset for the classification model
# 2 - testing dataset for the classification model
# 3 - held-out testing dataset
score_to_stratum={
    1:0,
    2:3,
    3:1,
...
class PairConfiguration:
    """Configuration class used to construct different dataset splits

    The class splits and filters image pairs and assigns them to different
    dataset splits according to user-given parameters
    """
    def __init__(self,
        ...
        """
        Args:
        label_json_paths: paths to jsons containing label information (e.g. ['data/TAO/train.json',...])
        categories_json_path: path to json containing category information (e.g. 'data/TAO/categories.json')
        count_instances: boolean determining whether n will determine number of object instances or object
                         categories (e.g. if count_instances is set to False, n=[2] will return all images with
                         exactly two object categories present, such as an image of five cars and three people,
                         whereas if count_instances is set to True, n=[2] will return all images with exactly
                         two object instances present, such as an image of two cars)
        k: number of most frequent categories that will be considered content (e.g. k=10 will use the 10 most
           frequent categories as content categories), if k is None, all categories will be considered content
        n: number of content or instance categories that will be present in an image pair (e.g. n=2 will return
           all image pairs with two content or instance categories present), if n is None, all image pairs will
           be returned
        content_categories: list of content categories, to be used if content and style categories are known
        style_categories: list of style categories, to be used if content and style categories are known
        prefixes: prefixes of the paths to the images (e.g. ['train', 'val', 'test'])
        nstrata: number of strata to split the dataset into
        strata_weights: weights of the strata, used to determine how many image pairs will be sampled from each
                        stratum (e.g. if strata_weights=(3,1,1,1), three times as many image pairs will be sampled
                        from stratum 0 as from stratum 1), the weighting is not exact, but rather a guideline
        """
        ...
        if content_categories is None and style_categories is None:
            ...
            raise RuntimeError("Both content and style need to be given or not given")
    ...
    def sample_pairs(self, stratum = 0):
        """
        Returns dataframe of pairs of images and corresponding content and
        style information.
        """
        data = []
    ...
    def _load_labels(self) -> Tuple[pd.DataFrame, list[int]]:
        """
        Constructs a dataframe with rows as individual frames from the TAO dataset
        and columns giving information about the sequence the image is from.
        The "list_object" column is a list containing all categories present in
        the image.
        """
        all_dfs = []
        all_categories = []
    ...
    def _load_categories(self) -> Dict[int, str]:
        """
        Returns a mapping from category identifiers to human-readable
        names
        """
    ...
    def _map_content_combinations_to_images(self) -> Dict[frozenset[int], list[str]]:
        """
        Constructs a dictionary mapping strata to a dictionary mapping all
        content combinations present in the stratum to all images displaying
        that content combination
        """
...
#methods for visualizing the dataset
